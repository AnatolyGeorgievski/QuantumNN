...
Маски вероятностей, как получить из изображения? (L)GBB
Причем тут регрессия?

## Гомография изображений и отслеживание объектов

Гомография может быть построена по набору ключевых точек на двух изображениях. Ключевые точки могут быть выделены с помощью методов OpenCV: SURF, ORB, AKAZE. Совмещение ключевых точек изображений выполняется методом RANSAC. Гомография может быть использована для вычисления глубины изображения. 
...
* [[1703.06870](https://arxiv.org/pdf/1703.06870)] Mask R-CNN -- мне нравится результат - это комбинация двух сверточных сетей: для детектирования и для выделения маски
...
HOG: Метод использует простые свертки (производные) $[-1,0,1]$ и $[-1,0,1]^T$ для вычисления градиентов ${g_x, g_y}$. Представление маски может быть в виде цветовой схемы HSV (Magnitude + angle).
...
* [[2502.20204](https://arxiv.org/pdf/2502.20204)] GRANITE EMBEDDING MODELS
...
Заметим, что статистически нормальным будет распределение случайной величины при бесконечном числе опытов. Формула получается как предельный переход (Предельная теорема Муавра-Лапласа). При конечном числе опытов имеет место биномиальное распределение, которое соотвествует статистике испытаний Бернулли. По аналогии с формулой биномиального распределения вводятся полиномы Бернштейна, на которых можно строить KSN (сплайновые сети Колмогорова). Формула бернулли как и полиномы Бернштейна выводятся через реккурентное соотношение, с фиксированной вероятностью реализации события при каждом испытаии.
...
$k_{1}=0{,}01$ и $k_{2}=0{,}03$ — эмпирические константы.
...
```
-- аналогично тому как складываются проводимости при вычислении сопротивления проводников. Мне хочется сравнить результат с методом "contrastive". 
...
* (https://habr.com/ru/articles/801885/) KNN метод ближайших соседей 
