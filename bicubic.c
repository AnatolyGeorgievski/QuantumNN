...
)
{
	return  v[1] + 0.5f * x * ((v[2] - v[0]) +
		x * (2.f *  v[0] - 5.f * v[1] + 4.f * v[2] - v[3] +
		//x * (1.f * (v[0] - v[1]) - 4.f * (v[1] - v[2]) + (v[0]-v[3]) +
		x * (3.f * (v[1] - v[2]) + (v[3] - v[0]))));
}
/*!
...
или выбрать фрагмент изображения
 */ 
    ...
    int n_channels)
{
    ...
    for(j=0; j<ne0; j++)
    {
        ...
        for(k=0;k<nc;k++)
        {
            ...
            {
                uint8_t d[4];
                d[0] = src[(y-0+r)*stride + (x+0)*nc +k];
                d[1] = src[(y-0+r)*stride + (x+1)*nc +k];
                d[2] = src[(y-0+r)*stride + (x+2)*nc +k];
                d[3] = src[(y-0+r)*stride + (x+3)*nc +k];
#if 0
                ...
                c[r] = (a[0]*u[0] + a[1]*u[1] + a[2]*u[2] + a[3]*u[3])/2;
#endif
                //c[r] = cubic_interp(d, dx);
            }
            float b[4];
            b[0] =           2*c[1];
            b[1] = -1*c[0]          + 1*c[2];
            ...
            float Cc = (b[0]*v[0] + b[1]*v[1] + b[2]*v[2] + b[3]*v[3])/2;
//            float Cc = cubic_interp(c, dy);
            // совместить с нормализацией: (x-mean)/std
            // разделить на планы по цветам
            dst[i*nb1 +j*nc +k] = fminf(255.f,fmaxf(0.f, Cc));
        }
       // dst[i*nb1 +j*nc +3] = 255;
    }
}
