...
* $f(V B V^{−1}) = V f(B) V^{-1}$, где $V$ - невырожденная (ортогональная) матрица. Это свойство следует обобщить на произвольную аналитическую функцию на единчином интервале и доказать как теорему. 
...
$$\min\limits_{x\in \mathbb{R^n}} \|Ax-b\|_2$$

* При выборе функции $\phi(x) = \|Ax-b\|_2^2$ можно использовать метод графдиентного спуска для поиска минимума $\nabla \phi(x) = 0$.
* Другое важно свойство квадратичной нормы - она не меняется при ортогональных преобразованиях. Может быть использована эквивалентная задач поиска минимума нормы $\|(Q^{\mathsf{T}}A)x - Q^{\mathsf{T}}b\|_2$


**Algorithm 5.3.1 (Normal Equations)**
Метод решения эквивалентной задачи минимизации нормы $\|(A^{\mathsf{T}}A)x - A^{\mathsf{T}}b\|_2$ с использованием разложения Холецкого для симетричных матриц.

**Algorithm 5.3.2 (Householder LS Solution)**
Метод решения задачи минимизации нормы $\|Ax - b\|_2$ с использованием QR разложения.
