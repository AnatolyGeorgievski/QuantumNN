...
Полярное разложение — представление квадратной матрицы $A$ в виде произведения эрмитовой $H$ и унитарной $W$ матриц $A=WH$. Является аналогом разложения любого комплексного числа в виде $z=|z|e^{i\varphi}$. Вещесвтенный аналог - разложение на симметричную и ортогональную матрицы.

В терминах SVD разложения, произвольная матрица $m \times n$ имеет разложение
$$A = U\Sigma V^\ast = (UV^\ast)(V\Sigma V^\ast) = WH~,$$
которое можно переписать $H = V\Sigma V^\ast$,  $W=UV^\ast$, где $U,V,W$ - унитариные (ортогональные) матрицы (для ясности $V^\ast V=I$), $H$ - эрмитова матрица. 
...
При переходе к вещесвтенным матрицам, Жордановы блоки содержащие комплексные ($\lambda = a+jb$) и комплесно-сопряженные ($\bar{\lambda}$) собственаные значения  будут представлены матрицамии $2 \times 2$ вида 
```math
\begin{bmatrix*}[r]
a& b\\
-b&a
...
Любое вращение можно представить, как поворот вокруг вектора нормального к плоскости вращения. Отражение двух векторов одинаковой длины можно определить плоскостью. Обобщение на случай n-мерного пространства, дает преобразование Гивенса и Хаусхолдера. Принцип поворота и отражения работает без измерения, плосктось в n-мерном пространстве называется гиперплоскостью.
...
см. Frobenius norm (Норма евклидого простарснтва, $L_2,2$)
