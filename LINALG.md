...
Я хотел бы иметь сборник освоенных алгоритмов, или готовую библиотеку методов. 
Но глядя в исходники готовых библиотек, я все равно ищу что-то свое 
в этом. Сейчас мне важно сделать свой инструментарий
эффективным. Я хочу научиться раскладывать большие матрицы, 
...
операторов в терминах BLAS и перенос вычислений на высокопроизводительные архитектуры
...
* Оценка и компенсация ошибки округления. Квантизация. 
...
Это требует 
методов разложения матриц: ортогонализация, диагонализация, балансировка, вращение; 
QR разложения, LDL - разложения Холе́цкого, SVD разложения, LS. В курсе 
следует уделить внимание методам классификации матриц и оптимизации для 
...
Линейная алгебра может быть приложением аналитической геометрии и теории линейных операторов. Теория линейных операторов начинается с теории функций [1] и продолжается в [2]. 
...
Следует обратить внимание на матрицы квази-диагональные, для которых на диагонали располагаются элементы 1х1 и 2x2 (соответствующие двум комплексно-сопряженным значениям). Путем вращений можно такую матрицу преобразовать к виду бидиагональной матрицы с ненулевыми элементами на диагонали и над диагональю. 
...
Характеристическим называется многочлен $f(\lambda) = |A - \lambda I|$. Следствие: у подобных матриц совпадают следы и определители.
...
(экспоненциальная матрица) $e^A$ - это матричная функция от квадратной матрицы, которая определяется степенным рядом следующим образом:
```math
...
Свойство разложение аналитических матричных функций
* $f(V B V^{−1}) = V f(B) V^{-1}$, где $V$ - невырожденная (ортогональная) матрица. Это свойство следует обобщить на произвольную аналитическую функцию на интервале и доказать как теорему.

**Матричные возмущения**
Теория возмущений оперирует понятием разложение в ряд тейлора. Ряд тейлора можно примнить для коммутативных и анти-коммутативных алгебр. Можно обобщить на случай алгебр с операцией коммутации, если определена операция - коммутатор. Приложением матричных функций являются численные вариационные методы и методы дискретные, которые требуют повышения точности (см. производная в конечных приращениях, остаточный член в форме Лагранжа). 
...
$det G≠0$ простой структуры (см. [Гант], гл. 3, §8, стр.85), не имеющая кратных точек спектра { } 1 2
 $λ_1,...,λ_{2n}$, $λ_i ≠ λ_j$, если i≠j, ij =1:2n.
...
$$a_i = \cfrac{\langle w, v_i \rangle}{\| w_i \| \| v_i \|}$$
...
2. Рассчитать для всех столбцов $$u_k = x_k - \sum_{j=1}^{k-1} \frac{\langle x_k, u_j \rangle}{\| u_j \|^2} u_j$$
...
Существует множество методов для решения задач разлодения
...
* Algorithm 5.2.6 (Modified Gram-Schmidt)  <!-- $2mn^2$ flops -->
...
см. Golub & van Loan[4]. 
* Algorithm 1.1.1 (Dot Product) $\alpha = x^T y$
...
**Алгоритм 1.1 (Dot Product)** $c = x^T y$, где $x, y \in \mathbb{R}^n$
```py
c = 0
for i=1:n :
   c = c + x[i]·y[i]
...
**Алгоритм (Вычисление коэффицентов матрицы вращения Гивенса)**
```py
def givens(a,b):
   if b==0:
      c=1; s=0
...
Заметим интересное свойство разложения, которое может быть использовано при оптимизации матричных операций. Если $x_t = A^t x_0$, то $x_t = V\Lambda^t V^{-1} x_0$. Причем степнная функция от диагональной матрицы означает возвещение каждого элемента в степень независимо от других. 
