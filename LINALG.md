...
```math
S=U^{\mathsf{T}}DU\quad {\text{где}}\quad D=\operatorname {diag} (\lambda _{1},\ldots ,\lambda _{n},\lambda _{1}^{-1},\ldots ,\lambda _{n}^{-1})~,
```
...
Для квадратной матрицы число обусловленности $k_2(Q) = \Vert A\Vert_2 \cdot \Vert A^{-1}\Vert_2$ выражается через ноорму матрицы. Если число обусловленности мало, то говорят, матрица хорошо-обусловленна.  Для ортогональной матрицы $Q$, $κ_2(Q) = \| Q \|_2 \cdot \Vert Q^T \Vert _2 = 1$.
