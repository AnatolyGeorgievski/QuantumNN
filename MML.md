...
Под квантизацией понимают представление чисел в формате чисел с плавающей точкой меньшей разрядности мантиссы или в целых (рациональных) числах. 
...
Пакет BLAS (пакет базовых подпрограмм линейной алгебры) включает набор масштабируемых операций с матрицами. Операции третьего уровня построены с использованием рекурсии, т.е. на каждом шаге алгоритма матрица разбивается на четыре подматрицы и вычисление происходит на матрицах меньшего размера. Задается порог, после которого операция считается неделимой. Эти методы подходят для больших матриц. При переносе на архитектуру GPU можно определить соответствующий порог для применения рекурсии. 
...
```math
\sigma (z)_{i} = \frac {e^{z_{i}}}{\sum _{k = 1}^{n} e^{z_{k}}}
```
...
$$\mathop{\rm{lse}}(z) = \ln \sum\limits_{i=1}^n e^{z_i}$$

$$\sigma (z)_{i} = {\partial \over \partial z_i}\mathop{\rm{lse}}(z)$$

Функцию softmax рекомендуется применять в сдвинутом виде, суть не меняется, но удается избежать переполенения при вычислении экспоненты. Смещение может быть константой, средним значением или нормой вектора. 

```math
\sigma (y)_{i} = \frac {e^{y_{i} - \max(y)}}{\sum _{k = 1}^{n} e^{y_{k}-\max(y))}}
```
Вектор-столбец $z=y-\max(y)$

Другой подход - нормировка вектора.
```math
\sigma (y)_{i} = \frac {e^{y_{i} / {\Vert y \Vert}}}{\sum _{k = 1}^{n} e^{y_i / {\Vert y \Vert}}}
```
