...
```
   ...
   │  │    ┌mantissa
   │┌─┴─┐┌─┴──────┐
 0b0000000000000000 IEEE binary8
```

Суффиксы при обозначении операций mixed-precision:
| суф.| формат| описание
|:---:|:-----:|:------------------
| F64 | E11M52| floating-point 64-bit (IEEE 754 double-precision float) (sign + exp11 + mant52)
| F32 | E8M23 | floating-point 32-bit (IEEE 754 single-precision float) (sign + exp8 + mant23)
| TF32| E8M10 | TensorFlow 32-bit
| BF16| E8M7  | floating-point 16-bit for machine learning ("bfloat16"). (sign + exp8 + mant7)
| F16 | E5M10 | floating-point 16-bit (sign + exp5 + mant10)
| BF8 | E5M2  | floating-point 8-bit : (sign + exp5 + mant2)
| F8  | E4M3  | floating-point 8-bit (also called "FP8"): (sign + exp4 + mant3)
...
Поддержка операций в Intel GPU
...
## Алгоритмы понижения разрядности

Постановка задачи: ускорить исполнение операции матричного умножения, за счет векторных упакованных форматов пониженной разрядности. 
Так если принять, что операции над числами FP8 выполняются в x32-x64 раза быстрее, чем векторные операции FP32, то такое представление становится оправданным. 


* [[2209.05433](https://arxiv.org/pdf/2209.05433)] FP8 FORMATS FOR DEEP LEARNING, 2022
* [[2203.03341](https://arxiv.org/pdf/2203.03341)] Recovering single precision accuracy from Tensor Cores while surpassing the FP32 theoretical peak performance, 2022
* [[RDNA4](https://www.amd.com/content/dam/amd/en/documents/radeon-tech-docs/instruction-set-architectures/rdna4-instruction-set-architecture.pdf)] RDNA4 Instruction Set Architecture: Reference Guide, 27-February-2025
* [[2412.19437](https://arxiv.org/pdf/2412.19437)] DeepSeek-V3 Technical Report, 2024
* [[2409.12517](https://arxiv.org/pdf/2409.12517)] M. Fishman, B. Chmiel, R. Banner, and D. Soudry. Scaling FP8 training to trillion-token llms. arXiv preprint arXiv:2409.12517, 2024.
* [[2210.17323](https://arxiv.org/pdf/2210.17323)] GPTQ: ACCURATE POST-TRAINING QUANTIZATION FOR GENERATIVE PRE-TRAINED TRANSFORMERS
* [[2211.10438](https://arxiv.org/pdf/2211.10438v6)] SmoothQuant: Accurate and Efficient Post-Training Quantization for Large Language Models,2022
* [[2306.11987](https://arxiv.org/pdf/2306.11987)] Training Transformers with 4-bit Integers, 2023
* [[2208.09225](https://arxiv.org/pdf/2208.09225)] FP8 Quantization: The Power of the Exponent, 2022

$$v_{F16} ← \mathsf{toFP16} (v_{F32}) $$
$$\Delta v_{F16} ← \mathsf{toFP16} (v_{F32} − \mathsf{toFP32} (v_{F16})).$$
Квантование модели с понижением разрядности F32 в  {.F16+\Delta.F16}
```c
half2 quantize_2f16(float * x) {
    uint ix = get_sub_group_local_id();
    float v = x[ix];
    return (half2){v, v-convert_half(v)};
}
```
...
На платформе `x86_64` относительно высокая производительность модели на GPU может быть получена в системе команд с разрядностью векторных регистров 512 бит:
* `AVX512_FP16` -- поддержка операций Float16, и скалярного произведения векторов;
* `AVX512_BF16` -- поддержка операций BFloat16, и скалярного произведения векторов;
