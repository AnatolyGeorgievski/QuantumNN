...
Поиск простых чисел для NTT. Для простого числа должен существовать корень степени $n$ из единицы, то есть $\omega^n \equiv 1 \mod q$. Для расчетов понадобится $\gamma^2 \equiv \omega \mod q$.

Критерием пригодности простого числа является возможность извлечения квадратного корня из $-1$, то есть $x^2 \equiv -1 \mod q$.

Мне удалось извлечь корень сепени n=16,32, 256,512 из некоторых простых чисел вида $A\cdot 2^{16}+1$ или $2^{32} - a0\cdot 2^{16}+1$. В литературе встречал числа вида $p = q\cdot 2^s+1$, где q - простое нечетное число. 
...
-- дан алгоритм выбора чисел $\omega$ и $\gamma$ для NTT через квадратичные не-вычеты. 
...
**Основные выводы по разделу**. Мы можем предложить функцию возвращающую некриптографический хэш от тензора используя модульную арифметику на кольце. В тоже время есть возможность составить криптографический хэш на модульной арифметике, примером является Poseidon2. При переводе матрицы в модульную арифметику, мы можем использовать NTT для ускорения операции умножения.
...
1. $\gamma ← 2$
3. while $(\gamma^2)^{2^s} \neq 1 \pmod{q}$ or $(\gamma)^{2^s} = 1 \pmod{q}$ do $\gamma ← \gamma + 1$
4. return $\gamma$
---
Заметитм в пункте (2) $(\gamma)^{2^s} = \pm 1 \pmod{q}$ - принимает одно из двух значений. Поиск значения $\gamma$ выполняется последовательным переборов $\gamma$ начиная с 2 или методом умножения $\gamma = \gamma \cdot g$ начиная с $g$.
