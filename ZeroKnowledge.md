...
1. function Partial_Reduction(𝑑, 𝑞, 𝑘, 𝑄=32, 𝐿=64)
2. $𝑐_1 ← 𝑑 ≫ Q$
3. $𝑐_2 ← 𝑐_1\cdot 𝑘 + d$
4. $𝑐_3 ← 𝑐_2 ≫ (L-Q)$
5. $𝑐_4 ← 𝑑 − 𝑞𝑐_3$
...
Расчет сдвиговой константы для выполнения операции folding.   

для случая $a_1\beta_1 == a\beta$, $\beta_1 = 2^{N_1}$ - величина сдвига, $N_1<N$
1. $k_1 ← \lfloor d/a_1 \rfloor$
2. $\{x_i,c_i\}  ← \{d - k_1 \cdot a_1, k_1\}$
3. $(x_{i} + c_{i}a_1)\cdot \beta_1 \mod (a_1\beta_1 -1) \equiv x_i\beta_1 + c_i$

Замена деления на умножение и сдвиг.
