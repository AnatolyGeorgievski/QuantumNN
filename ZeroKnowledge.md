...
\end{aligned}
```
При этом получаем целочисленный метод квантизации подобные `Q8`. 
Немного иначе будет выглядеть квантизация для MX - multiscale типов, для которых выделяется общая экспонента `E8M0`. Операция - округление RTE с насыщением, финитная арифметика.
...
Использование методов гомоморфного шифрования требует стандартизации методов сериального представления тензоров, методов канонизации бинарного описания тензоров и графа тензорных операций нейросети. Стандартизация и канонизация рассматривается в контексте RPC протокола. Для реализации бинарного RPC протокола подходят стандарты Google Protobuf и CBOR. Протоколы RPC могут быть использованы не только в задачах распределения вычислений в кластере, но и Edge-computing, таких как распределенная обработка визуальных данных. 
...
> четко даются определения и математические основы полностью гомоморфного преобразования на единичном бублике.
...
$Z[X]/(XN + 1)$. $\mathbb{T}_N [X]$ denotes $\mathbb{R}[X]/(X^N + 1) \mod 1$ and $\mathbb{B}_N[X]$ denotes the polynomials in $\mathbb{Z}_N[X]$ with binary coefficients.

**Определение (R-module)**. Let $(\mathcal{R}, +, ×)$ be a commutative ring. We say that
...
* обратный элемент по сложению - отрицание $c_i = q - a_i$ 
...
* произведение векторов $a, b \in \mathcal{R}_q$, 
```math
...
corresponds to polynomial-polynomial multiplication in $\mathbb{Z}^𝑁_q /(𝑋^𝑁 − 1)$, whereas HE operates in $\mathcal{R}_𝑞 = \mathbb{Z}^𝑁_𝑞/(𝑋^𝑁 + 1)$. В этом случае умножение принимает вид 

$$\tilde{с} = \mathrm{𝐼𝑛𝑣𝑁𝑇𝑇} (\mathrm{𝑁𝑇𝑇}(\tilde{𝑎}) \odot \mathrm{𝑁𝑇𝑇}(\tilde{𝑏}))$$

где элементы векторов $\tilde{a}, \tilde{b}, \tilde{с}$ домножаются на вектор корней полинома, $\tilde{a} = \{a_0, a_1\psi, a_2\psi^2, ... a_{N-1}\psi^{N-1}\}$, $\psi$ - примитивный корень полинома N-й степени.
...
Техники, такие как алгоритмы Кули-Тьюки (Cooley-Tukey Radix-2 NTT) и Джентльмена-Санда  (Gentleman-Sande (GS) Radix-2 InvNTT), а также тщательный выбор параметров (например, $N$ как степень 2 и $q$, поддерживающее эффективное NTT), улучшают производительность.
