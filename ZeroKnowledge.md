...
* (qnn_hexl.c) 
-- алгоритмы для схемы FHE Ring-LWE с оптимизацией под AVX512. Оптимизация под 32 битные простые числа вида $A\cdot 2^{16} - 1$ и модули NIST PQC.
...
* Модульная арифметика на кольце полиномов $\mathbb{Z}_q/\langle x^N + 1\rangle$ может быть представлена, как тензорные операции: ADDM, SUBM, INVM, MULM, DOTM, MVM (умножение матрицы на вектор). 
