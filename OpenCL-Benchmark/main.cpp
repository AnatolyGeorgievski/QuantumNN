...
string fraction(const float x) {
	float values[]   = { 1.0f/64.0f, 1.0f/32.0f, 1.0f/24.0f, 1.0f/16.0f, 1.0f/12.0f, 1.0f/8.0f, 1.0f/4.0f, 1.0f/3.0f, 1.0f/2.0f, 2.0f/3.0f, 
		1.0f, 2.0f, 3.0f, 4.0f, 8.0f, 12.0f, 16.0f, 32.0f, 64.0f };
	string strings[] = {   "1/64"  ,   "1/32"  ,   "1/24"  ,   "1/16"  ,   "1/12"  ,   "1/8 ",   "1/4 ", "1/3 ", "1/2 ", "2/3 ", 
		" 1x ", " 2x ", " 3x ", " 4x ", " 8x ", " 12x", " 16x", " 32x", " 64x" };
	uint i=0u, imin=0u;
...
void benchmark_device(const Device_Info& device_info) {
	...
	double time_long=max_double, time_int=max_double, time_short=max_double, time_char=max_double, time_sum=max_double, time_mod=max_double;
	...
	const float flops_short = 4096.0f*(float)N/(float)time_short*1E-12f;
	...
	const float flops_char = 2*4096.0f*(float)N/(float)time_char*1E-12f;
...
//  George 2024-2025
	...
	time_sum=max_double;
	print("| Benchmarking ...                                                            |");
	Kernel kernel_mwc(device, N, "kernel_mwc", buffer);
	for(uint i=0u; i<N_kernel; i++) {
		clock.start();
		kernel_mwc.run();
		time_sum = fmin(clock.stop(), time_sum);
	...
	flops_sum = 1024.0f*(float)N/(float)time_sum*1E-12f;
	println("\r| MWC   compute "+alignr(45u, to_string(flops_sum, 3u))+"  TBOPs/s "+fraction(100.0f*flops_sum/device.info.tflops)+" |");
	time_sum=max_double;
	print("| Benchmarking ...                                                            |");
	...
	time_sum=max_double;
	print("| Benchmarking ...                                                            |");
	Kernel kernel_mod(device, N, "kernel_mod_uint", buffer);
	for(uint i=0u; i<N_kernel; i++) {
		clock.start();
		kernel_mod.run();
		time_sum = fmin(clock.stop(), time_sum);
	...
	flops_sum = 1024.0f*(float)N/(float)time_sum*1E-12f;
	println("\r| MOD   compute "+alignr(45u, to_string(flops_sum, 3u))+"  TBOPs/s "+fraction(100.0f*flops_sum/device.info.tflops)+" |");
	time_sum=max_double;
	print("| Benchmarking ...                                                            |");
	Kernel kernel_tnn(device, N, "kernel_tnn_uint", buffer);
	for(uint i=0u; i<N_kernel; i++) {
		clock.start();
		kernel_tnn.run();
		time_sum = fmin(clock.stop(), time_sum);
	...
	time_sum=max_double;
	print("| Benchmarking ...                                                            |");
	Kernel kernel_mma(device, N, "kernel_mma", buffer);
	for(uint i=0u; i<N_kernel; i++) {
		clock.start();
		kernel_mma.run();
		time_sum = fmin(clock.stop(), time_sum);
	...
	flops_sum = 16*1024.0f*(float)N/(float)time_sum*1E-12f;
	println("\r| MMA.h compute "+alignr(45u, to_string(flops_sum, 3u))+"  TIOPs/s "+fraction(100.0f*flops_sum/device.info.tflops)+" |");
// ~George

	print("| Benchmarking ...                                                            |");
	Kernel kernel_block_write(device, N, "kernel_block_write"  , buffer);
	time_cw = max_double;
	for(uint i=0u; i<N_kernel; i++) {
		clock.start();
		kernel_block_write.run();
		time_cw = fmin(clock.stop(), time_cw);
	}
	Kernel kernel_block_read(device, N, "kernel_block_read"  , buffer);
	time_cr = max_double;
	for(uint i=0u; i<N_kernel; i++) {
		clock.start();
		kernel_block_read.run();
		time_cr = fmin(clock.stop(), time_cr);
	}
	println("\r| Memory Bandwidth ( block read          ) "+alignr(29u, to_string(4.0f*(float)N*(float)M/(float)(time_cr-time_cw/(double)M)*1E-9f, 2u))+" GB/s |");
	println("\r| Memory Bandwidth ( block write         ) "+alignr(29u, to_string(4.0f*(float)N*(float)M/(float)(time_cw)*1E-9f, 2u))+" GB/s |");
