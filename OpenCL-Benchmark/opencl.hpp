...
public:
	...
	inline Device(const Device_Info& info, const string& opencl_c_code=get_opencl_c_code()) {
		...
		const string build_options = "-cl-std=CL2.0 -cl-finite-math-only -cl-no-signed-zeros -cl-mad-enable";//+(info.patch_intel_gpu_above_4gb ? " -cl-intel-greater-than-4GB-buffer-required" : "");
#ifndef LOG
