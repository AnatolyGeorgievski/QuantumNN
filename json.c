/*! JSON (JavaScript Object Notation) разбор текстового формата
	...
	\author Anatoly Georgievskii <>, 2024
 ...
 */
/*! \brief Освобождает память, выделенную под дерево JSON с дочерними элементами. 

    Все строки считаем выделенными динамически. Удаляем строки с использованием g_free() */
...
/*! \brief освобождает память, но не удаляет строки */
void json_free_antistatic(JsonNode* js)
{
    ...
    if (js->type==JSON_ARRAY || js->type==JSON_OBJECT){
        if (js->value.list) {
            GSList* list= js->value.list;
            while (list){
                if (list->data) json_free_antistatic((JsonNode*)list->data); //list->data = NULL;
                list = list->next;
            }
            g_slist_free(js->value.list);
            js->value.list=NULL;
...
/*! \brief выделить строку
    \param[IN] s - начало строки
    \param[OUT] tail - остаток строки, заканчивается `"`
...
/*! \brief выполнить разбор строки в формате JSON 
    \param[IN] s - начало позиции разбора на входе
    \param[OUT] tail - остаток строки после разбора элемента JSON
    \param[OUT] error - ошибка при разборе или NULL
    \return указатель на разобранный элемент JSON

    \note Строка должна начинаться с элемента JSON: `"`, `[`, `{`, `true`, `false`, `null`, число или `-`
 */
JsonNode *json_value(char* s, char** tail, GError** error)
{
	char *start = s;//
	char *s_end = (tail != NULL)? *tail: NULL;
    JsonNode* js = NULL;
	...
	if (s_end) 
		while (s<s_end && g_ascii_isspace(s[0]))s++;
	else
		while (g_ascii_isspace(s[0]))s++;
do_exit:
    if(tail)*tail = s;
...
#define OFFS_INC 1
/*! \brief Вывод массива JSON в текстовом читаемом виде с отступами и переносами строк
    \param list -- список элементов массива
    \param str -- строка, в которую записывается форматированное представление
	\param offset -- отступ слева при форматировании строк
...
void json_array_to_string(GSList* list, GString* str, int offset)
{
	g_string_append(str, "[");
	if (list){
		json_to_string((JsonNode*)list->data, str, offset+OFFS_INC);
		list = list->next;
		while (list) {
			g_string_append(str, ",");
			json_to_string((JsonNode*)list->data, str, offset+OFFS_INC);
...
/*! \brief Вывод в текстовом читаемом виде с отступами и переносами строк
    \param js -- элемент JSON
	\param offset -- отступ слева при форматировании строк
 */
void json_to_string(JsonNode* js, GString* str, int offset)
{
    ...
    case JSON_ARRAY: {
        ...
        g_string_append_printf(str, "%*s]", 0/*offset*/, "");
    } break;
    case JSON_OBJECT: {
        GSList* list = js->value.list;
        g_string_append(str, "{\n");
...
/*! \brief получить элемент списка - по идентификатору 
    \return элемент списка (JSON_OBJECT)
 */
